...
fn main() {
    ...
    for candidate in (5..loop_end).step_by(2) {
        if check_prime(candidate, &primes) {
            ...
            if primes.len() as u32 >= args.prime_goal { break; }
    ...
    fn minor_report(minor_interval: u32, minor_timer: &mut Stopwatch, last_index: usize, prime_array: &Vec<u32>) {
        if minor_interval == 0 { return; }
    ...
    fn major_report(major_interval: u32, major_timer: &mut Stopwatch, last_prime: u32) {
        ...
        let last_text: String = numlang::get_name(major_interval);
